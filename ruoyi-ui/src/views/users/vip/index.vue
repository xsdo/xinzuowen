<template>
  <div class="app-container">
    <el-form :model="queryParams" ref="queryForm" size="small" :inline="true" v-show="showSearch" label-width="68px">
      ...
      <el-form-item label="会员起始时间" prop="vBegintime">
        <el-date-picker clearable
          v-model="queryParams.vBegintime"
          type="date"
          value-format="yyyy-MM-dd"
          placeholder="请选择会员起始时间">
        </el-date-picker>
      </el-form-item>
      <el-form-item label="会员到期时间" prop="vEndtime">
        <el-date-picker clearable
    ...
    <el-table v-loading="loading" :data="vipList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="会员起始时间" align="center" prop="vBegintime" width="180">
        <template slot-scope="scope">
          <span>{{ parseTime(scope.row.vBegintime, '{y}-{m}-{d}') }}</span>
        </template>
      </el-table-column>
      <el-table-column label="会员到期时间" align="center" prop="vEndtime" width="180">
        <template slot-scope="scope">
...
<script>
import { listVip, getVip, delVip, addVip, updateVip } from "@/api/users/vip";
...
export default {
  ...
  methods: {
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除用户会员编号为"' + ids + '"的数据项？').then(function() {
        return delVip(ids);
      }).then(() => {
        this.getList();
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.download('users/vip/export', {
        ...this.queryParams
